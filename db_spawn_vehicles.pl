...
my %db = (
	'host' => $args{'hostname'} ? $args{'hostname'} : '209.222.93.26',
	'instance' => $args{'instance'} ? $args{'instance'} : '1',
	'limit' => $args{'limit'} ? $args{'limit'} : '5000',
	'user' => $args{'username'} ? $args{'username'} : 'sequisha',
	'pass' => $args{'password'} ? $args{'password'} : 'ahsiuqes',
	'name' => $args{'database'} ? $args{'database'} : 'dayztest',
	'port' => $args{'port'} ? $args{'port'} : '3316',
);
